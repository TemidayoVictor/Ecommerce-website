...
@section('content')
    @if ($pageTitle == 'Kitchen Item')
        <div class="product-continer admin-section">
            ...
            <form action="{{ route('admin.edit-products', ['id' => $product->id]) }}" method="post" enctype="multipart/form-data">
                @csrf

                <div class="input-field" style="display: none">
                    <label for=""> <h4>Category</h4> </label>
                    <input type="text" name="category" value="{{ $category->id }}" required>
                </div>
